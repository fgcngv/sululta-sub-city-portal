


const om = {
  hero: {
    badge: "Sululta Daawwadhaa",

    texts: [
      "Baga gara Magaalaa Sulultaa Nagaan dhuftan",
      "Uumama Sulultaa Daawwadhaa",
      "Muuxannoo adda ta'e argadhaa",
      "Imalli keessan as irraa jalqaba",
      "Sululta fooyya'aa ijaaraa jirra",
    ],

    description:
      "Namoota, iddoowwan, kalaqa, aadaa fi muuxannoowwan Sulultaa argadhaa.",

    button: "Sululta Daawwadhaa",

    nature: "Uumama fi Imala",

    science: "Saayinsii fi Teeknooloojii",

    culture: "Hawaasa fi Aadaa",
  },

  administration: {
    eyebrow: " Hoggansa Keenya",
    title: "Bulchiinsa ",
    description:
      " Bulchitoota ummata Magaalaa Sulultaa tajaajiluu fi hawaasa keenya tarkaanfachiisuuf hojjetan waliin wal baraa.",
    administrationLabel: " ",
    mainAdministrator: " Bulchiinsa",
    fromAdministration: "Bulchiinsa irraa",
    welcomeTitle: " Baga Gara Kutaa Magaalaa Sulultaatti Nagaan  dhuftan",
  
    welcomeParagraph1:
      " Bulchiinsi Magaalaa Shaggar kaayyoo cimaa hawaasa keenya fuulduratti tarkaanfachiisuuf qabnu qooduuf gammachuu guddaatu ​​nutti dhagahama. Shaggar gara Magaalaa Ismaartii fakkeenyummaa qabdutti jijjiiruuf kutannoo cimaa qabna. Magaalaa dorgomtuu, jireenya gaarii dhiyeessitu, fi hawaasa ishee hundaaf deebii kennitu akka taatufis ni hojjenna.",
  
    welcomeParagraph2:
      " Galmi keenya Shaggar magaalaa kompaaktii fi wiirtuu dinagdee giddugaleessa hedduu qabu gochuudha. Kunis hirmaannaa hawaasaa, walitti hidhamiinsa tarsiimoo, fi bulchiinsa naannoo cimsuun kan argamu ta’a.",
  
    welcomeParagraph3:
      " Kana malees, tarsiimoo qindoomina qabuu fi bu’uuraalee misoomaa haaraa fi ismaartii hawaasni keenya barbaadu irratti invastimantii xiyyeeffannoo qabu irratti ni hojjenna.",
  
    governorName: "Obbo Kumaa Hundee",
    governorRole: "Bulchaa Kutaa Magaalaa Sulultaa",
  
    administrators: [
      {
        name: "Obbo Kumaa Hundee",
        role: "Bulchaa Kutaa Magaalaa Sulultaa",
        message:
          " Hoggansa cimaa fi hirmaannaa hawaasaatiin Magaalaa Sulultaa ammayyaa, hunda hammatte, fi badhaate ijaaruuf waadaa seennee jirra.",
      },
      {
        name: "Aadde Waaziraa Jamaal I/A Bulchaa Kutaa Magaalaa Sulultaa",
        role: "I/A Bulchaa ",
        message:
          "Hawaasa keenya waliin ta’uun tajaajila fooyyessuu fi jiraataa hundaaf carraa fooyya’aa uumuu. ",
      },
      {
        name: "Obbo Amantee Fullaasaa Bulchaa Aanaa Abbaa Gadaa",
        role: "Bulchaa Aanaa Abbaa Gadaa",
        message: "",
      },
      {
        name: "Obbo Shimallis Nagaash Bulchaa Aanaa Wasarbii",
        role: "Bulchaa Aanaa Wasarbii",
        message: "",
      },
      {
        name: "Obbo Taarraqeny Gaddafaa Itti Gaafatamaa Waajjira Saayinsii fi Teeknooloojii Kutaa Magaalaa Sulultaa",
        role: "",
        message:
          "Jijjiirama dijitaalaa fooyyessuuf hawaasa keenya waliin hojjenna.",
      },
      {
        name: "I/G Kominikeeshinii Kutaa Magaalaa Sulultaa",
        role: "I/G Kominikeeshinii Kutaa Magaalaa Sulultaa",
        message: "",
      },
      {
        name: "I/G Waajjira Kadastaraa Kutaa Magaalaa Sulultaa",
        role: "I/G Waajjira Kadastaraa Kutaa Magaalaa Sulultaa",
        message: "",
      },
      {
        name: "I/G Waajjira Maallaqaa Kutaa Magaalaa Sulultaa",
        role: "I/G Waajjira Maallaqaa Kutaa Magaalaa Sulultaa",
        message: "",
      },
    ],
  },

  overview: {

    badge:" Kutaa Magaalaa Sulultaa",

    title:" Hawaasa guddachaa jiru tajaajiluu",

    description:
      " Odeeffannoo waa’ee tajaajila mootummaa, misoomaa, jalqabbii hawaasaa fi hojii Bulchiinsa Magaalaa Sulultaa argadhaa.",


    administrator:{
      label:"",

      heading:
      "Dhaamsa I/G waajjira Saayinsii fi Teeknooloojii Kutaa Magaalaa  Sulultaa ",

      name:"Obbo Taarraqeny Gaddafaa ",

      role:
      "I/G waajjira Saayinsii fi Teeknooloojii Kutaa Magaalaa  Sulultaa",

      welcome:
      " Baga Gara Magaalaa Sulultaatti Nagaan dhuftan",


      shortMessage:
      " Duraan dursee nagaa fi kabaja guddaa isiniif qaba. Saayinsii fi teeknooloojii fayyadamuun tajaajila mootummaa ammayyeessuu, hojii saffisaa fi qulqullina qabu kennuu, akkasumas kalaqaa fi yaada haaraa jajjabeessuun...",


      fullMessages:[
        "Duraan dursee nagaa fi kabaja guddaa isiniif qaba. Saayinsii fi teeknooloojii fayyadamuun tajaajila mootummaa ammayyeessuu, hojii saffisaa fi qulqullina qabu kennuu, akkasumas kalaqaa fi yaada haaraa jajjabeessuun mul’ata keenya keessaa isa ijoodha.",

        "Waajjirri Saayinsii fi Teeknooloojii Kutaa Magaalaa Sulultaa, dandeettii raawwachiisummaa cimsuu, dargaggootaa fi ogeessotaaf carraa kalaqaa uumuu, teeknooloojii haaraa hojii keessa galchuu fi tajaajila dijitaalaa babal’isuuf hojjechaa jira.",
        
        "Hojii keenya keessatti milkaa’inni argamu hirmaannaa, tumsa fi yaada uummataa malee guutuu ta’uu hin danda’u. Kanaaf, hundi keenya saayinsii, teeknooloojii fi kalaqa akka carraa misoomaa fi jijjiirama waaraa ta’etti fayyadamuun gahee keenya haa bahannu.",
        
        "“Saayinsii fi Teeknooloojii – Bu’uura Jijjiiramaa fi Misooma Magaalaa Keenyaa!”",
        
        "Galatoomaa. Itti Gaafatamaa Waajjira Saayinsii fi Teeknooloojii Kutaa Magaalaa Sulultaa"
      ],


      expandButton:"Ergaa Hunda",

      collapseButton:"Xiqqeessi"
    },


    cards:{
      development:{
        label:"Guddina",
        title:" Pirojektoota fi Bu'uuraalee misoomaa",
        description:
        " Hojiiwwan misoomaa fi jalqabbii bu’uuraalee misoomaa guutuu Sulultaatti qorachuu. ",
        button:" Gadi fageenyaan "
      },

      education:{
        label:"Barnoota",
        title:"Barnoota",
        description:
        " Manneen barnootaa, tajaajila barnootaa fi jalqabbii barnoota hawaasaa argadhu.",
        button:"Gadi fageenyaan"
      }
    }

  },

  projects:{
    label:" Pirojektoota Keenya",
  
    title:
    " waggoota shanan darban keessatti Pirojektoota xumuraman",
  
    description:
    " Pirojektoota misoomaa waggoota dhiyoo asitti xumuraman tokko tokkoo fi dhiibbaa isaan hawaasaa fi bu’uuraalee misoomaa irratti qaban qorachuu.",
  
  
    viewAll:
    " Pirojektoota hunda ilaali",
  
    viewGallery:
    " Suuraawwan Ilaali",
  
  
  
    items:[
  
      {
        id:"project-1",
        title:"Gamoo Waajjira Bulchiinsa Kutaa Magaalaa Sulultaa",
        year:2026,
        location:"Addis Ababa",
        description:"",
        image:"/images/projects/img-1.png",
        href:"/projects/community-development"
      },
  
  
      {
        id:"project-2",
        title:"Kutaa Magaalaa Sulultaatti Staadiyeemii Spoortii",
        year:2025,
        location:"Ethiopia",
        description:"",
        image:"/images/projects/img2.png",
        href:"/projects/urban-infrastructure"
      },
  
  
      {
        id:"project-3",
        title:"Kutaa Magaalaa Sulultaatti Gamoo Buufata Fayyaa",
        year:2024,
        location:"Ethiopia",
        description:
        "",
        image:"/images/projects/img4.png",
        href:"/projects/rural-development"
      },
  
  
      {
        id:"project-4",
        title:"Kutaa Magaalaa Sulultaatti Gamoo Giddugala Gabaa",
        year:2023,
        location:"Ethiopia",
        description:
        " Ijaarsa fi misooma dhaabbilee ummataa.",
        image:"/images/projects/img3.png",
        href:"/projects/public-facility"
      },
  
  
      {
        id:"project-5",
        title:"Karaa Keessa Keessaa",
        year:2022,
        location:"Ethiopia",
        description:"",
        image:"/images/projects/img5.png",
        href:"/projects/infrastructure-improvement"
      }
  
    ]
  },

  news: {

    label:
      "Oduu Haaraa",
  
    title:
      "Oduu fi Beeksisa",
  
    description:
      "Oduu haaraa, hojiiwwan fi beeksisa mootummaa Kutaa Magaalaa Sulultaa hordofaa.",
  
    viewAll:
      "Oduu hunda ilaali",
  
  },

  agriculture: {

    label:
      "Qonna",
  
    title:
      "Inisheetiivota misooma qonnaa",
  
    description:
      "Inisheetiivota qonnaan bultoota deeggaru, oomisha qonnaa guddisu, jallisii fi horsiisa beeyladaa Sululta keessatti hojjetaman qoradhaa.",
  
    viewAll:
      "Inisheetiivota hunda ilaali",
  
    clickView:
      "Suuraa ilaaluuf cuqaasi",
  
    close:
      "Daawwannaa suuraa cufi",
  
    previous:
      "Inisheetivii darbe",
  
    next:
      "Inisheetivii itti aanu",
  
    learnMore:
      "Dabalataan baradhu",
  
  },

  contact: {

    eyebrow:
      "Nu qunnami",
  
    title:
      "Bulchiinsa Magaalaa Sulultaa waliin qunnamaa",
  
    description:
      "Odeeffannoo, tajaajila mootummaa fi gaaffilee hojii mootummaa irratti bulchiinsa waliin qunnamaa.",
  
  
    address:{
      label:"Waajjira",
      value:"Bulchiinsa Kutaa Magaalaa Sulultaa",
    },
  
  
    phone:{
      label:"Bilbila",
      value:"+251961083131",
      href:"tel:+251961083131",
    },
  
  
    email:{
      label:"Imeelii",
      value:"sulultssubcity@gmail.com",
      href:"mailto:sulultssubcity@gmail.com",
    },
  
  
    officeHours:{
      label:"Sa'aatii hojii",
      value:"Wiixata hanga Jimaata, 2:30 AM – 11:30 PM",
    },
  
  
    mapTitle:
      "Bakka Bulchiinsa Kutaa Magaalaa Sulultaa",
  
    mapHeading:
      "Kaartaan bakka jiru",
  
    mapDescription:
      "Odeeffannoon bakka mootummaa yeroo dhiyaatu itti dabalamuu danda'a.",
  
  },

  about: {
    hero: {
      badge: "Waa'ee Sulultaa",
      title: "Hawaasa keenya tajaajiluu.",
      subtitle: "Fuuldura keenya ijaaruu.",
      description:
        "Bulchiinsi Kutaa Magaalaa Sulultaa tajaajila mootummaa gaarii fi bulchiinsa itti gaafatamummaa qabuun hawaasa cimsaa, walitti dhufeenya qabu fi badhaadhaa ta'e ijaaruuf kutannoo qaba.",
    },
  
  
    introduction: {
      eyebrow: "Nuti eenyu?",
      title: "Bulchiinsa ummataa fi guddina irratti xiyyeeffate.",
  
      paragraphs: [
        "Sululti hawaasa guddachaa jiru, eenyummaa cimaa, carraa fi itti gaafatamummaa waliinii qabu dha.",
  
        "Tajaajila mootummaa, bu'uuraalee misoomaa, hirmaannaa hawaasaa fi bulchiinsa gaarii irratti hundaa'uun jireenya guyyaa guyyaa fooyyeessuuf hojjenna.",
      ],
  
      commitmentTitle: "Kutannoo keenya",
  
      commitment:
        "Tajaajila fooyya'aa, hawaasa cimaa fi misooma naannoo itti fufiinsa qabu ijaaruu.",
    },
  
  
    direction: {
      eyebrow: "Kallattii keenya",
  
      title: "Kaayyoo ifaa irratti hundaa'uun hogganamuu.",
  
      visionTitle: "Mul'ata keenya",
  
      vision:
        "Sulult magaalaa hawaasa hunda hammattu, badhaatuu, tajaajila saffisaa fi itti fufiinsa qabu magaalaa kennitu akka taatu gochuu.",
  
      missionTitle: "Ergama keenya",
  
      mission:
        "Tajaajila mootummaa bu'a qabeessa, salphaa fi itti gaafatamummaa qabu kennuun guddina naannoo deeggaruu.",
    },
  
  
    values: {
      eyebrow: "Waan nu qajeelchu",
  
      title: "Duudhaalee hojii keenya duuba jiran.",
  
      description:
        "Hojii keenya tajaajila, itti gaafatamummaa fi ummata Sulultaa waliin hojjechuu irratti hundeessina.",
  
  
      items: [
        {
          title: "Hawaasaf dursa kennuu",
  
          description:
            "Fedhii, hawwii fi nageenya hawaasa magaalattii hojii keenya keessatti bakka jalqabaa irra keenya.",
        },
  
  
        {
          title: "Itti gaafatamummaa",
  
          description:
            "Bulchiinsa iftoomina qabu, amanamaa fi itti gaafatamummaa qabu kennuuf hojjenna.",
        },
  
  
        {
          title: "Tajaajila qulqullina qabu",
  
          description:
            "Tajaajilawwan mootummaa salphaa, saffisaa fi namni hundi argachuu danda'u gochuuf hojjenna.",
        },
      ],
    },
  
  
    closing: {
      eyebrow: "Bulchiinsa Kutaa Magaalaa Sulultaa",
  
      title:
        "Waliin taanee Sululta cimaa ijaarra.",
  
      description:
        "Odeeffannoo argadhaa, tajaajila mootummaa fayyadamaa akkasumas bulchiinsa keessan waliin wal qunnamaa.",
  
      button:
        "Oduu haaraa",
    },
  },

  contactUs: {
    hero: {
      eyebrow: "Bulchiinsa Kutaa Magaalaa Sulultaa Quunnamaa",
      title: "Nu as jirra si gargaaruuf.",
      description:
        "Gaaffii qabduu, tajaajila mootummaa irratti odeeffannoo barbaaddaa, ykn bulchiinsa waliin qunnamuu barbaaddaa? Nu qunnamaa.",
    },
  
    details: {
      office: {
        title: "Waajjira keenya daawwadhaa",
        value: "Bulchiinsa Kutaa Magaalaa Sulultaa",
        description: "Sulultaa, Oromiyaa, Itoophiyaa",
      },
  
      phone: {
        title: "Nuuf bilbilaa",
        value: "+251961083131",
        description:
          "Wiixata – Jimaata, yeroo hojii keessatti",
      },
  
      email: {
        title: "Imeelii nuuf ergaa",
        value: "sulultssubcity@gmail.com",
        description:
          "Gaaffii waliigalaa fi odeeffannoo tajaajila mootummaa",
      },
  
      hours: {
        title: "Yeroo hojii",
        value: "Wiixata – Jimaata",
        description: "8:30 WB – 5:30 WB",
      },
    },

    form: {
      getInTouch: "Nu Qunnamaa",
      sendMessage: "Ergaa nuuf ergaa",
      sendMessageDescription:
        "Unka kana fayyadamuun gaaffii keessan Nuuf ergaa. Gareen keenya gaaffii keessan sirriitti hubatee deebii akka kennuuf odeeffannoo gahaa nuuf kennaa.",
  
      urgentTitle: "Dhimmoota ariifachiisaa",
      urgentDescription:
        "Unki quunnamtii kun gaaffilee  waliigalaa irratti kan xiyyeeffatedha. Haala hatattamaa ykn gargaarsa ariifachiisaa barbaaduuf qaama dhimmicha ilaalu qunnamaa.",
  
      name: "Maqaa Guutuu",
      namePlaceholder: "Maqaa Keessan Galchaa",
  
      email: "Iimeelii",
      emailPlaceholder: "you@example.com",
  
      phone: "Lakk. Bilbilaa",
      phonePlaceholder: "+251 ...",
  
      subject: "Mata-Duree",
      subjectPlaceholder: "Maal isin gargaaruu dandeenya?",
  
      message: "Ergaa",
      messagePlaceholder: "Ergaa keessan galchaa...",
  
      privacyNotice:
        "Odeeffannoo dhuunfaa miira namaa kakaasu kan gaaffii keessaniif barbaachisaa hin taane hin galchinaa.",
  
      send: "Ergaa Ergi",
      sending: "Ergaa Jira...",
  
      successTitle: "Ergaan keessan milkaa'inaan ergameera",
      successDescription:
        "Nu quunnamuu keessaniif galatoomaa.",
  
      errorTitle: "Ergaan keessan hin ergmne",
      errorDescription:
        "Maaloo xiqqoo turaa irra deebi'aa yaalaa.",
    },
  
    location: {
      title: "Nu daawwadhaa",
      address: "Bulchiinsa kutaa magaalaa sulultaa",
      city: "Sululta, Oromiyaa, Itiyoophiyaa",
  
      officeHours: "Yeroo Hojii",
      weekdays: "Wiixata – Jimaataa",
      time: "2:30 AM – 11:30 PM",
    },
  },


  education: {
  hero: {
    badge: "Barnootaa fi Barachuu",
    title: "Barnootni",
    subtitle: "Egeree keenya ijaaruuf Nu Gargaara.",
    description:
      "Daa'immanii fi dargaggoota Kutaa Magaalaa Sulultaa keessatti argamaniif barnoota, beekumsa fi carraa deeggaruu.",
    exploreSchools: "Mana barnootaa ilaali",
    viewGallery: "Suuraa ilaali",
    imageAlt: "Barnootaa fi barachuu ",
  },

  introduction: {
    eyebrow: "Barnootaa fi carraa",
    title: "Naannoo barnootaa cimaa ijaaruu",
    paragraphs: [
      "Barnootni guddina magaalaa keessatti gahee murteessaa qaba. Manneen barnootaa mootummaa fi dhuunfaa Kutaa Magaalaa Sulultaa keessatti sadarkaalee barnootaa adda addaa irratti daa'immanii fi dargaggootaaf carraa barnootaa ni kennu.",
      "Bulchiinsi naannoo qulqullina barsiisotaa, naannoo barnootaa nageenya qabu, bu'uuraalee barnootaa, hirmaannaa maatii fi hawaasaa, akkasumas hirmaannaa uummataa cimsuun barbaachisaa ta'uu ni hubata.",
    ],
  },

  statistics: {
    eyebrow: "Daataa barnootaa",
    title: "Staatistiksii Barnootaa",
    description:
      "Daataan barnootaa jiru haala barnootaa fi reeshiyoo barataa fi barsiisaa manneen barnootaa mootummaa keessatti mul'atu agarsiisa.",
    items: [
      {
        value: "37:1",
        label: "Reeshiyoo barataa fi barsiisaa KG",
        description:
          "Manneen barnootaa mootummaa Kutaa Magaalaa Sulultaa keessatti gabaafame.",
      },
      {
        value: "37:1",
        label: "Reeshiyoo barataa fi barsiisaa sadarkaa 1ffaa",
        description:
          "Kutaa 1–8 manneen barnootaa mootummaa keessatti.",
      },
      {
        value: "34:1",
        label: "Reeshiyoo barataa fi barsiisaa sadarkaa 2ffaa",
        description:
          "Kutaa 9–12 manneen barnootaa mootummaa keessatti.",
      },
    ],
    source:
      "Madda: Waajjira Karooraa fi Misoomaa Bulchiinsa Magaalaa Shaggar, 2024. Lakkoofsotni kun manneen barnootaa mootummaa Kutaa Magaalaa Sulultaa qofa ilaallatu.",
  },

  levels: {
    eyebrow: "Daandii barnootaa",
    title: "Barnoota sadarkaalee adda addaa",
    description:
      "Barnootni Sulultaa keessatti :  barnoota daa'immanii, barnoota sadarkaa 1ffaa, sadarkaa 2ffaa fi carraa barnoota itti fufaa of keessatti hammata.",
    prePrimary: {
      title: "Barnoota Duraa fi Oolmaa Daa'immanii",
      description:
        "Barnootni daa'immanii dandeettii fi ofitti amanamummaa daa'immanii guddisee gara barnoota sadarkaa 1ffaatti akka ce'an isaan qopheessa.",
    },
    primary: {
      title: "Barnoota Sadarkaa 1ffaa",
      description:
        "Barnootni sadarkaa 1ffaa kutaa 1–8 kan hammatu yoo ta'u, bu'uura barnoota itti aanuuf barbaachisaa dha.",
    },
    secondary: {
      title: "Barnoota Sadarkaa 2ffaa",
      description:
        "Barnootni sadarkaa 2ffaa barattoota kutaa ol'aanaa keessatti argaman deeggaruun barnoota itti aanuuf fi hojii jireenyaaf isaan qopheessa.",
    },
    higher: {
      title: "Barnoota Itti Fufaa",
      description:
        "Barattoonni koolleejjii, yunivarsiitii fi carraalee barnoota biroo irratti barnoota isaanii itti fufuu danda'u.",
    },
  },

  publicSchools: {
    eyebrow: "Mana barnootaa mootummaa",
    title: "Barnoota mootummaa Sulultaa keessatti",
    description:
      "Manneen barnootaa mootummaa daa'immanii fi dargaggoota Kutaa Magaalaa Sulultaa keessatti argamaniif barnoota dhaqqabamaa ta'e kennuudhaan gahee guddaa qabu.",
    imageAlt: "Manneen barnootaa mootummaa muraasa suuradhaan ",
    cardTitle: "Mana barnootaa mootummaa",
    cardDescription:
      "Manneen barnootaa mootummaa sirna barnootaa mootummaa keessatti tajaajila barnootaa kennuun hawaasaalee Kutaa Magaalaa Sulultaa keessatti argaman tajaajilu.",
    noteTitle: "Barnoota mootummaa",
    noteDescription:
      "",
  },

  privateSchools: {
    eyebrow: "Barnoota dhuunfaa",
    title: "Manneen barnootaa dhuunfaa muraasa suuradhaan",
    description:
      "Manneen barnootaa dhuunfaa maatiiwwaniif filannoo barnoota dabalataa kennuun carraa barnootaa garaagaraa babaldhisuu keessatti gahee ni taphatu.",
    badge: "Barnoota dhuunfaa",
    imageAlt: "Mana barnootaa dhuunfaa Sululta keessatti",
    imageLabel: "Mana barnootaa dhuunfaa",
    cardTitle: "Dhaabbata barnootaa dhuunfaa",
    cardDescription:
      "Manneen barnootaa dhuunfaa naannoo barnootaa, bu'uuraalee, sagantaalee fi hojiiwwan dabalataa adda addaa qabaachuu danda'u.",
    featured: {
      eyebrow: "Mana barnootaa filatamaa",
      title: "School of Success",
      description:
        "School of Success, kan magaalaa Sulultaa keessatti argamu, kutaa barnootaa banaa, teeknooloojii smart-board, sagantaalee dubbisuu fi herregaa, ispoortii fi leenjii IT qabaachuu isaa odeeffannoo maxxanse keessatti ibseera.",
      imageAlt:
        "Barattoota School of Success Sulultaa keessatti barachaa jiran",
      features: [
        "Bara 2014 hundaa'e",
        "Kutaa barnootaa banaa",
        "Teeknooloojii smart-board",
        "Dubbisuu fi herrega",
        "Hojiiwwan ispoortii",
        "Leenjii IT",
      ],
      button: "Odeeffannoo mana barnootaa",
      href: "https://www.ethioharvardschool.com/schoolofsuccess.php",
    },
  },

  initiatives: {
    eyebrow: "Inisheetiivota barnootaa",
    title: "Hojiilee fi jijjiirama barnootaa yeroo dhiyoo",
    description:
      "Fakkeenyonni kun hojiiwwan fi fooyya'iinsa barnootaa Sulultaa keessatti gabaafaman agarsiisu.",
    category: "Barnoota",
    sourceLabel: "Madda",
    items: [
      {
        title: "Paarkii Dubbisaa Mana Barnootaa Sadarkaa 2ffaa Sulultaa",
        date: "2024",
        description:
          "Gudina Tumsa Foundation paarkiin dubbisaa Mana Barnootaa Sadarkaa 2ffaa Sulultaa keessatti banamuu isaa gabaaseera. Paarkichi barattootaaf bakka dubbisaaf mijataa ta'e kenna.",
        image: "/images/projects/img52.png",
        imageAlt:
          "Paarkii dubbisaa Mana Barnootaa Sadarkaa 2ffaa Sulultaa",
        source: "Gudina Tumsa Foundation, December 2024",
      },
      {
        title: "Fooyya'iinsa Tajaajila WASH Mana Barnootaa Babo Wale Ilu",
        date: "2024",
        description:
          "Mana Barnootaa Babo Wale Ilu keessatti tajaajilli qulqullinaa, bakka harka dhiqannaa fi sirni kuusaa bishaanii fooyya'uu isaa gabaafameera.",
        image: "/images/projects/img444.png",
        imageAlt:
          "Fooyya'iinsa tajaajila WASH Mana Barnootaa Babo Wale Ilu",
        source: "Gudina Tumsa Foundation, December 2024",
      },
    ],
  },

  gallery: {
    eyebrow: "Kuusaa suuraa barnootaa",
    title: "Naannoo barnootaa fi jireenya mana barnootaa",
    description:
      "Suuraalee manneen barnootaa, naannoo barnootaa fi hojiiwwan barnootaa Sulultaa keessatti gaggeeffaman ilaali.",
    imageAlt: "Barnoota Sulultaa keessatti",
  },

  research: {
    eyebrow: "Qorannoo fi barnoota",
    title: "Barnoota Sulultaa hubachuu",
    description:
      "Qorannoon akademikaa fi qorannoowwan naannoo wantoota bu'aa barnootaa fi hirmaannaa mana barnootaa irratti dhiibbaa qaban hubachuuf gargaara.",
    study: {
      title: "Qorannoo manneen barnootaa sadarkaa 2ffaa",
      description:
        "Qorannoon bara 2025 gaggeeffame tokko hirmaannaa maatii fi milkaa'ina barattootaa manneen barnootaa sadarkaa 2ffaa Kutaa Magaalaa Sulultaa keessatti qorateera.",
      button: "Qorannoo dubbisi",
      href: "https://journals.wgu.edu.et/index.php/star/article/view/1952",
    },
  },

  cta: {
    title: "Barnootni fuuldura Sulultaa ijaara",
    description:
      "Manneen barnootaa cimaa, naannoo barnootaa deeggaru fi hirmaannaan hawaasaa carraa barnootaa fooyya'aa uumuuf gahee guddaa qabu.",
    button: "Bulchiinsa qunnami",
  },
},


};

export default om;

